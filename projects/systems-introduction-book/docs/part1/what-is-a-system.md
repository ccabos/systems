...
**Where do the rules belong?** In an organisation the connections are not abstract. They are written down: a constitution, a career and tenure regime, a budget ordinance, signature-authority limits, an objectives-and-targets scheme, reporting duties. Because these are concrete objects with a file name, it is tempting to count them among the parts — and that mistake inverts the table, filing the expensive, high-consequence elements under "most easily swapped".

A written rule is **physical in form and a connection in function**: what it does is decide which part is coupled to which, in which direction, under which conditions. Classify it by what it does. The practical difference is large. A part can be replaced over a weekend and little follows; a rule takes years to change, is owned by someone other than whoever wants it changed, and moves the system's behaviour when it moves.

There is a second consequence, and it is the reason bureaucracy is worth defending. A system is dependable exactly to the degree that its connections do not yield on request — equal treatment, predictability and reviewability are not side effects of an organisation's rules, they are what the rules *are*. The same property makes it slow to change. Resistance to change is not a defect to engineer out; it is the price of the property that makes the system worth having.
...
This hierarchy maps directly onto the SE decomposition used throughout this book, with one caution. Physical implementations (P) hold two different kinds of thing: the parts themselves, and the written rules that carry the connections. Logical architecture and functions (L, F) describe the connection pattern in the abstract; the P-level rule documents are how it is actually enforced. Goals and requirements (G, R) encode purpose. Institutional reform that touches only parts is maintenance; reform that changes the rules is not. The distinction is not academic: in the worked democracy decomposition, `RP1` is "Parliament building, rules of procedure" and `RP2` is "Constitution, bill of rights" — both physical-level, but the first is largely parts and the second is entirely connection. Reading the whole physical level as maintenance would make a constitutional amendment a routine swap of the state's most easily replaced element. Reform that reaches G-level is transformation — and the historical record shows it is both rare and destabilising.
